...
anova_data <- g1_data %>%
  select(starts_with("IV"), DV_ban, gender_mfnb, political_party_dri, age, region_txt, manipulation_check, prior_knowledge) %>%
...
library("ggpubr")
ggboxplot(anova_data, x = "IV_org", y = "DV_ban", color = "IV_stat",
          palette = c("#00AFBB", "#E7B800"))
ggboxplot(anova_data, x = "gender_mfnb", y = "DV_ban",
          palette = c("#00AFBB", "#E7B800"))
...
aov_results <- aov(DV_ban_num ~ IV_org * IV_stat + gender_mfnb + political_party_dri + region_txt + manipulation_check + prior_knowledge, data = anova_data)
summary(aov_results)

# manipulation check as outcome
aov_results <- aov(manipulation_check_num ~ IV_org * IV_stat + gender_mfnb + political_party_dri + region_txt + prior_knowledge, data = anova_data)
summary(aov_results)
  
# ordered logistic regression -- https://stats.oarc.ucla.edu/r/dae/ordinal-logistic-regression/
olr_model <- MASS::polr(DV_ban_3 ~ IV_org * IV_stat + gender_mfnb + political_party_dri + region_txt + manipulation_check + prior_knowledge, data = anova_data, Hess=T)
summary(olr_model)
